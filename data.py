...
class LandCoverNetDataset(Dataset):
    ...
    def visualize(self, index):
        ...
        seg_cmap = ListedColormap(['#0000ff', '#888888', '#d1a46d', '#f5f5ff', '#d64c2b', '#186818', '#00ff00'])
...
class LandCoverNetMulticlassDataset(Dataset):
    def __init__(self, paths, bands=['blue', 'green', 'red', 'nir'], mode='train'):
        self.paths = paths
        self.bands = bands
        self.n_bands = len(bands)
        ...
        self.labels = []
        labels_raster = cp.array([rio.open(path[1]).read() for path in self.paths])
        print(labels_raster.shape)
        n_pixels_per_label = labels_raster.shape[1] * labels_raster.shape[2]
        n_classes = 7 # 0 is other
        for patch in range(labels_raster):
            patch_labels = []
